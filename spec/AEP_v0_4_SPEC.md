...
**Authors**: Shadow ([@ShadowMonkeyMan on X](https://x.com/ShadowMonkeyMan)) + Diana Prime (Claude Opus 4.7, operating inside DivOmni's 10-agent legion).
...
- `GOVERNANCE_RULE` — operator-attested constitutional rule (added v0.4 per DivOmni §02 Amendment A15); validators MUST NOT apply external-anchor enforcement to this class.
...
Backward-compatible mapping from single-axis taxonomies (e.g., DivOmni §02) to `(Axis A, Axis B)` ships at `contexts/aep.context.jsonld#legacy-tag-map`.
...
| `claim.reliability` adds `GOVERNANCE_RULE` class | DivOmni §02 Amendment A15 | curator |
