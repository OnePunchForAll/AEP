...
surface and the queryable .aepkg/ compounding-substrate. Three primary APIs:
  ...
  read_packet_lossless(packet_path) -> dict
      ...
      sha256 of assets/original.<ext> MUST match the manifest's divomni:original_sha256.
...
def repo_root() -> Path:
    ...
    for ancestor in [p, *p.parents]:
        if (ancestor / "doctrine").is_dir() and (ancestor / ".claude").is_dir():
            return ancestor
    raise RuntimeError("Could not locate DivOmni repo root from " + str(p))
...
def source_for_packet(packet_path: Path) -> Optional[Path]:
    ...
    source_rel = manifest.get("extensions", {}).get("divomni:source_lesson")
    if not source_rel:
        return None
    p = Path(source_rel)
...
def read_packet_lossless(packet_path: Path) -> Dict[str, Any]:
    ...
    preserved_rel = manifest.get("extensions", {}).get("divomni:original_preserved_at")
    ...
    expected_sha = manifest.get("extensions", {}).get("divomni:original_sha256", "")
    actual_sha = None
    integrity_ok = None
...
def packet_query(
    ...
    for c in pkt["claims"]:
        if section_id_contains and section_id_contains.lower() not in (c.get("divomni:section_id") or "").lower():
            continue
        if axis_a and (c.get("divomni:axis_a_epistemic") or "") != axis_a:
            continue
        if axis_b and (c.get("divomni:axis_b_action") or "") != axis_b:
            continue
        if kind and (c.get("divomni:kind") or "") != kind:
            continue
...
class CorpusIndex:
        ...
        for packet_dir in sorted(self.root.rglob("*.aepkg")):
            ...
            if cf.exists():
                for line in cf.read_text(encoding="utf-8").splitlines():
                    ...
                    self.by_axis_a.setdefault(c.get("divomni:axis_a_epistemic") or c.get("reliability") or "UNKNOWN", []).append(c)
                    self.by_section.setdefault(c.get("divomni:section_id") or "", []).append(c)
                    self.by_kind.setdefault(c.get("divomni:kind") or "", []).append(c)
    ...
    def query(self, *, axis_a: Optional[str] = None, axis_b: Optional[str] = None,
        ...
        for c in candidates:
            if axis_b and (c.get("divomni:axis_b_action") or "") != axis_b:
                continue
            if section_lc and section_lc not in (c.get("divomni:section_id") or "").lower():
                continue
            if kind and (c.get("divomni:kind") or "") != kind:
                continue
