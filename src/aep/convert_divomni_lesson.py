"""
convert_divomni_lesson.py — Convert a DivOmni lesson .html artifact into an AEP v0.3 .aepkg/

Phase-1.1 PERFECTED implementation.
Owner: forge.
Status: validator-clean (every output passes the operator's reference validator with ok=true).

Design fixes vs Phase-1 skeleton:
  ...
  - GOVERNANCE-RULE legacy tag round-trips through reliability=PROVEN_RELIABLE +
    divomni_legacy_tag=GOVERNANCE-RULE per doctrine/02 Amendment A15.
...
Usage:
    python -m aep.convert_divomni_lesson <lesson.html> <output.aepkg/>
"""
...
# Forward map: DivOmni single-axis tag -> (Axis A AEP reliability, Axis B action, AEP-compatible reliability)
# The third element is the AEP-validator-compatible value that goes into the `reliability` field.
# Axis A may carry a richer value (GOVERNANCE_RULE) preserved in divomni:axis_a_epistemic extension.
...
def map_truth_tag(legacy_tag: str) -> Tuple[str, str, str]:
    """Forward-map a DivOmni single-axis tag to (axis_a, axis_b, aep_reliability)."""
...
# --- HTML parser: extracts structured records from a DivOmni lesson ---
class DivOmniLessonParser(HTMLParser):
    """Parses a DivOmni lesson .html into sections, paragraphs, frontmatter, and cite links."""
...
# --- Markdown-wrapped-in-HTML parser (older DivOmni lessons store frontmatter in
...
# --- Frontmatter (YAML-ish) parser tuned for DivOmni lessons ---
...
def classify_source_type(href: str) -> str:
    ...
    if h.endswith(".html") and ("/doctrine/" in h or h.startswith("doctrine/") or "../doctrine/" in h or "../" in h):
        return "primary_source"  # DivOmni-internal canonical doctrine
...
def build_lesson_self_source(lesson_path: Path, lesson_title: str, nowiso: str) -> Dict[str, Any]:
    return {
        ...
        "limits": (
            "DivOmni-internal lesson; truth tag carried in frontmatter applies to the full document; "
    ...
    for sec in sections:
        ...
        for paragraph in sec["paragraphs"]:
            ...
            claim = {
                ...
                "status": "active",
                "created_at": nowiso,
                # DivOmni / V11-AEP extension fields (additionalProperties:true):
                "divomni:axis_a_epistemic": axis_a,
                "divomni:axis_b_action": axis_b,
                "divomni:legacy_tag": legacy_tag,
                "divomni:kind": kind,
                "divomni:section_id": sid,
                "divomni:section_title": title,
                "divomni:strong_lead": strong_lead,
            }
            claims.append(claim)
...
def extract_relations(
    ...
    for claim in claims:
        by_section.setdefault(claim["divomni:section_id"], []).append(claim["id"])
...
def emit_packet(
    ...
    events: List[Dict[str, Any]] = [{
        ...
        "type": "WriteEvent",
        "op": "create_packet",
        "actor": "forge:convert_divomni_lesson.py-Phase-1.1",
        "target": "aepkg.json",
        "pre_state_hash": "sha256:" + "0" * 64,
        "post_state_hash": "TBD-after-state-hash-computed",
        "rationale": "Phase-1.1 first validator-clean conversion of DivOmni lesson HTML to AEP v0.3 packet",
        "created_at": nowiso,
    }]
    ...
    initial_validation = [{
        ...
        "type": "ValidationRun",
        "validator": "convert_divomni_lesson.py-Phase-1.1",
        "result": "pass",
        "checked_files": list(REQUIRED_FILES) + ["aepkg.json"],
        "findings": [
            {"severity": "info", "path": "convert_divomni_lesson.py", "message": "Converter self-emits a pass-grade run; external reference validator is the gating check."}
        ],
    ...
    manifest: Dict[str, Any] = {
        ...
        "title": lesson_title or lesson_path.stem,
        "created_at": nowiso,
        "created_by": "forge:convert_divomni_lesson.py-Phase-1.1",
        "profile": "aep:0.3/minimal-jsonl",
        "canonical_files": list(REQUIRED_FILES),
        "extensions": {
            "divomni:source_lesson": str(lesson_path).replace("\\", "/"),
            "divomni:source_frontmatter": frontmatter,
            "divomni:legacy_truth_tag": legacy_tag,
            "divomni:two_axis_schema": "V11-charter-section-2.1-operator-approved-2026-05-14",
            "divomni:converter_phase": "Phase-1.1",
            "divomni:governance_rule_amendment": "doctrine/02 Amendment A15 operator-approved 2026-05-14",
        },
        "integrity": {
    ...
    manifest["extensions"]["divomni:original_preserved_at"] = f"assets/{preserved_filename}"
    manifest["extensions"]["divomni:original_sha256"] = original_sha
    manifest["extensions"]["divomni:original_bytes"] = len(original_bytes)
    with open(output_dir / "aepkg.json", "w", encoding="utf-8", newline="\n") as f:
        json.dump(manifest, f, sort_keys=True, indent=2, ensure_ascii=False)
        f.write("\n")
...
def write_views(
    ...
    summary = [
        ...
        f"**Source**: `{manifest['extensions']['divomni:source_lesson']}`  ",
        f"**State hash**: `{manifest['integrity']['state_hash']}`  ",
        f"**Claims**: {len(claims)} | **Sources**: {len(sources)} | **Relations**: {len(relations)}  ",
        f"**Legacy truth tag**: `{manifest['extensions']['divomni:legacy_truth_tag']}`  ",
        "",
        "## Claims (one per row)",
        "",
    ]
    for claim in claims[:80]:
        axis_a = claim.get("divomni:axis_a_epistemic", "?")
        axis_b = claim.get("divomni:axis_b_action", "?")
        sid = claim.get("divomni:section_id", "?")
        kind = claim.get("divomni:kind", "?")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    print(f"=== convert_divomni_lesson.py Phase-1.1 ===")
    ...
    if fmt == "markdown-wrapped":
        ...
        parser_inst = DivOmniLessonParser()
